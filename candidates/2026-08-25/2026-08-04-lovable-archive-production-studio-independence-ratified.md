---
id: 2026-08-04-lovable-archive-production-studio-independence-ratified
type: decision
status: candidate
source: this chat, 2026-08-04, Abad's decision after the archive was located (source status: active)
project: lords-of-cian
tags: [lords-of-cian, lovable, operator-contribution]
---

# Lords of Cian Archive and Production Studio Stay Fully Independent For Now

## Body

Abad decided to keep the Lovable-hosted Lords of Cian Archive and the STAG Production Studio as fully independent systems for now. The Fleet may periodically suggest whether the two should connect, including drafting a preliminary implementation thesis if it wants to, but actually wiring them together stays a future decision Abad has not made yet.

## Links

- derived-from: 2026-08-04-lords-of-cian-lovable-archive-located
